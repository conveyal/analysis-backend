    ...
    public void checkOpportunityExtents (PointSet travelTimePointSet) {
        if (calculateAccessibility) {
            for (PointSet opportunityPointSet : destinationPointSets) {
                if (!opportunityPointSet.getWebMercatorExtents().equals(travelTimePointSet.getWebMercatorExtents())) {
                    throw new AssertionError("Travel time would be calculated to a PointSet that does not match " + "the opportunity PointSet.");
                }
            }
        }
    }
